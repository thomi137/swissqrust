...
impl LayoutBlock for InformationBlock {
    ...
    fn render(&self, layout: &mut BillLayout, ops: &mut Vec<DrawOp>, cursor: &mut ColumnCursor) {
        ...
        // Account / Payable to
        draw_label(ops, label!(AccountPayableTo, layout.language), cursor.x, &mut cursor.y, layout.label_font_size, layout.line_spacing);
        ...
        cursor.advance(layout.text_ascender);
        draw_text_lines(ops, &layout.bill_data.creditor_address.to_lines().all_but_last(), cursor.x, &mut cursor.y, layout.text_font_size, layout.line_spacing, layout.extra_spacing);
        cursor.advance(layout.line_spacing);

        // Reference
        match &layout.bill_data.reference_type {
            ReferenceType::QrRef(reference) => {
                draw_label(ops, label!(Reference, layout.language), cursor.x, &mut cursor.y, layout.label_font_size, layout.line_spacing);
                cursor.advance(layout.text_ascender);
        ...
        if self.part == SlipPart::PaymentPart {
            if let Some(unstructured_message) = &layout.bill_data.unstructured_message {
                draw_label(ops, label!(AdditionalInformation, layout.language), cursor.x, &mut cursor.y, layout.label_font_size, layout.line_spacing);
                cursor.advance(layout.text_ascender);
        ...
        if let Some(debtor) = &layout.bill_data.debtor_address {
            draw_label(ops, label!(PayableBy, layout.language), cursor.x, &mut cursor.y, layout.label_font_size, layout.line_spacing);
            cursor.advance(layout.text_ascender);
            draw_text_lines(ops, &debtor.to_lines().all_but_last(), cursor.x, &mut cursor.y, layout.text_font_size, layout.line_spacing, layout.extra_spacing);
            cursor.advance(layout.extra_spacing);
        } else {
            draw_label(ops, label!(PayableByNameAddress, layout.language), cursor.x, &mut cursor.y, layout.label_font_size, layout.line_spacing);
            cursor.advance(layout.text_ascender);
